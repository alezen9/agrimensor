...
describe("resource tracking", () => {
  it("is available without a frame boundary", () => {
    ...
    expect(groma.capabilities.resourceTracking).toBe(true);
  });
  ...
  it("sums declared buffer sizes", () => {
    ...
    const { resources } = groma.snapshot();
  ...
  it("keeps the peak after resources are released", () => {
    ...
    expect(groma.snapshot().resources.liveResourceAllocationPeakInBytes).toBe(
      4096,
    );
  ...
  it("ignores resources created before attach", () => {
    ...
    expect(groma.snapshot().resources.liveBufferCount).toBe(0);
  });
